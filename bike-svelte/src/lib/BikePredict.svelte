<script>
    ...
    async function getPredict(ele){
        ...
        console.log(comJson)
        //모델과 같이 칼럼 순서를 맞춰야 함으로 순서를 맞춰줌
        //데이터 프레임에 칼럼을 정렬하여 쉽게 정렬할 수 있게 나중에 수정
        let comJsonSort={
                        "season":0,
                        "weather":0,
                        "temp":0,
        ...
        comJsonSort["season"]=comJson["season"]
        comJsonSort["weather"]=comJson["weather"]
        comJsonSort["temp"]=comJson["temp"]
        comJsonSort["atemp"]=comJson["atemp"]
        comJsonSort["humidity"]=comJson["humidity"]
        comJsonSort["year"]=comJson["year"]
        comJsonSort["hour"]=comJson["hour"]
        comJsonSort["dayofweek"]=comJson["dayofweek"]
        comJsonSort["holiday"]=comJson["holiday"]
        comJsonSort["workingday"]=comJson["workingday"]
        //모델 칼럼에 맞게 정렬 끝
        let comJsonSortArr=[comJsonSort]
        ...
        const res=await fetch(endpoint,{
            ...
            body:JSON.stringify(comJsonSortArr)
	    })
